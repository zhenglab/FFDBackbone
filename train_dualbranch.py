...
from engine_finetune import train_dual_one_epoch, test_dual_one_epoch
args = get_params()
setup(args)
init_exam_dir(args)
###########################
...
def main():
    ...
    criterion_e = losses.__dict__[args.loss.name](**(args.loss.params if getattr(args.loss, "params", None) else {})).cuda(args.local_rank)
    criterion_b = losses.__dict__[args.loss2.name](**(args.loss2.params if getattr(args.loss2, "params", None) else {})).cuda(args.local_rank)
    ...
    for epoch in range(start_epoch, num_epochs+1):
        ...
        train_dual_one_epoch(train_dataloader, model, criterion_b, criterion_e, optimizer, epoch, global_step, args, logger, lr_scheduler, num_epochs)
        global_step += len(train_dataloader)
        test_dual_one_epoch(test_dataloader, model, criterion_b, criterion_e, optimizer, epoch, global_step, args, logger,lr_scheduler, num_epochs)
