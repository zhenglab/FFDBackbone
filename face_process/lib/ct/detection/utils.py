...
def grab_interval_frames(path, max_size, cvt=False,n_frames=32,interval=5):
    capture = cv2.VideoCapture(path)
    ret = True
    frame_count_org = int(capture.get(cv2.CAP_PROP_FRAME_COUNT))
    frame_idxs = None
    
    if n_frames * interval > frame_count_org:
        frame_idxs = np.linspace(0, frame_count_org - 1, n_frames, endpoint=True, dtype=int)
    ...
    for cnt_frame in range(frame_count_org): 
        ...
        if ret:
            ...
            if cvt:
                frame = frame[..., ::-1]
            frames.append(frame)
            frames_ids.append(cnt_frame)
            cnt_frame += 1
            if len(frames) == max_size and max_size != -1:
                break

    capture.release()
    return frames,frames_ids

def grab_spase_frames(path, max_size, cvt=False,n_frames=32,interval=5):
    capture = cv2.VideoCapture(path)
    ret = True
    frame_count_org = int(capture.get(cv2.CAP_PROP_FRAME_COUNT))
    frame_idxs = None
    # n_frames = max_size
    if n_frames < frame_count_org:
        frame_idxs = np.linspace(0, frame_count_org - 1, n_frames, endpoint=True, dtype=int)
...
def bbox_range(bbox):
    face_range = (bbox[2]-bbox[0])*(bbox[3]-bbox[1])
    return face_range
